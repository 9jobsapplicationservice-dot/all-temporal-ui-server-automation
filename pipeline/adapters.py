...
def run_linkedin_stage(record: dict, python_executable: str | None = None) -> dict:
    ...
    env = {
        **runtime_env,
        "PIPELINE_MODE": "1",
        "PIPELINE_RUN_ID": record.get("id") or record.get("runId") or "",
        "PIPELINE_OUTPUT_DIR": record.get("run_dir") or "",
        "PIPELINE_APPLIED_CSV_PATH": record.get("applied_csv_path") or "",
        "PIPELINE_EXTERNAL_CSV_PATH": record.get("external_jobs_csv_path") or "",
        "PIPELINE_RECRUITERS_CSV_PATH": record.get("recruiters_csv_path") or "",
        "PIPELINE_FAILED_CSV_PATH": str(Path(record.get("log_dir") or "." ) / "failed_jobs.csv"),
        "PIPELINE_LOGS_DIR": record.get("log_dir") or "",
        "PIPELINE_SCREENSHOTS_DIR": str(Path(record.get("log_dir") or ".").parent / "screenshots"),
